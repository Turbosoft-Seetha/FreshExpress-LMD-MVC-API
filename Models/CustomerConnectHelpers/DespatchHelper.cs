using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;

namespace MVC_API.Models.CustomerConnectHelpers
{
    public class DespatchHelper
    {
    }
    public class DispatchIn
    {
        public string userID { get; set; }
        public string JSONStringCus { get; set; }
        public string JSONStringOutlet { get; set; }
        public string JSONStringRot { get; set; }
        public string JSONStringOrder { get; set; }
        public string JSONStringProducts { get; set; }
      //  public string JSONStringStatus { get; set; }
        public string FromDate { get; set; }
        public string ToDate { get; set; }
        public string Mode { get; set; }
    }
    public class DispatchOut
    {
        public string DispatchID { get; set; }
        public string Status { get; set; }
        public string ExpectedDelDate { get; set; }
        public string cus_ID { get; set; }
        public string cus_Code { get; set; }
        public string cus_Name { get; set; }
        public string cus_HeaderID { get; set; }
        public string cus_HeaderCode { get; set; }
        public string cus_HeaderName { get; set; }
        public string rot_ID { get; set; }
        public string rot_Code { get; set; }
        public string rot_Name { get; set; }
        public string ord_ERP_OrderNo { get; set; }
        public string ord_LPONumber { get; set; }
        public string ID { get; set; }
    }
    public class DispatchDetailIn
    {
        public string ID { get; set; }

    }
    public class DispatchDetailOut
    {

        public string prd_ID { get; set; }
        public string prd_Code { get; set; }
        public string prd_Name { get; set; }
        public string prd_Desc { get; set; }
        public string BaseUOM { get; set; }
        public string Qty { get; set; }


    }
    public class PostItemDespatchCus
    {
        public string cus_HeaderID { get; set; }

    }
    public class PostItemDespatchCusOutlet
    {
        public string ID { get; set; }

    }
    public class PostItemDespatchRot
    {
        public string rot_ID { get; set; }

    }
    public class PostItemDespatchOrder
    {
        public string ordNo { get; set; }
    }
    public class PostItemDespatchProducts
    {
        public string prd_ID { get; set; }
    }
    public class PostItemDespatchStatus
    {
        public string Status { get; set; }
    }
    public class DespatchCustomersIn
    {
        public string usrID { get; set; }
        public string FromDate { get; set; }
        public string ToDate { get; set; }
        public string Mode { get; set; }
    }
    public class DespatchCustomersOut
    {
        public string cus_HeaderID { get; set; }
        public string cus_HeaderCode { get; set; }
        public string cus_HeaderName { get; set; }
    }

    public class CFDispatchIn
    {
        public string userID { get; set; }
        public string JSONStringCus { get; set; }
        public string JSONStringOutlet { get; set; }
        public string JSONStringRot { get; set; }
        public string JSONStringOrder { get; set; }
        public string JSONStringProducts { get; set; }
    }
    public class CFDispatchOut
    {
        public string dsp_ID { get; set; }
        public string dsp_DispatchID { get; set; }
        public string ord_ERP_OrderNo { get; set; }
        public string CreatedDate { get; set; }
        public string ord_ExpectedDelDate { get; set; }
        public string dsp_ExpDeliveryDate { get; set; }
        public string csh_Code { get; set; }
        public string csh_Name { get; set; }
        public string cus_Code { get; set; }
        public string cus_Name { get; set; }
        public string rot_Name { get; set; }
        public string rot_Code { get; set; }
        public string DispatchedBy { get; set; }
        public string dsp_DispatchType { get; set; }
        public string TotalAmount { get; set; }
        public string ApprovedBy { get; set; }
        public string Status { get; set; }
        public string ord_LPONumber { get; set; }

    }
    
}